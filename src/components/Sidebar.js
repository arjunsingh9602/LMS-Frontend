    ...
    >
      ...
      <nav className="menu-container" style={{ paddingLeft: "10px", paddingRight: "10px" }}>
        {Menuitems.map((item) => {
          ...
          const isActive = pathname === item.href || (hasSubmenu && openSubmenu === item.label);
            ...
            <div key={item.label} className="menu-item" style={{ marginBottom: "8px" }}>
              {hasSubmenu ? (
                ...
                >
                  ...
                  <ChevronDown className={`menu-arrow transition-transform duration-200 ${openSubmenu === item.label ? "rotate-180" : ""}`} />
                </button>
              ) : (
                <Link
                  href={item.href}
